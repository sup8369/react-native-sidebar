...
export default class SidebarView extends Component {
	...
	close() {
		...
		Animated.timing(this.openProgress, { toValue: 0, duration: 100 }).start();
	}
	...
	_handleShouldMove(event, gestureState) {
		...
		if(locationX < 0) { locationX = 0; return; }
		this._progressOffset = this.openProgress._value;
		this._progressFactor = 1 / contentWidth;

		console.log(this._progressOffset + gestureState.dx * this._progressFactor)
		if(this._progressOffset + gestureState.dx * this._progressFactor > 0.79) return
	...
	_handleMove(event, gestureState) {
		var currentProgress = this._progressOffset + gestureState.dx * this._progressFactor;
		
		if(currentProgress < 0) { currentProgress = 0; return; } 
	...
	_handleMoveEnd(event, gestureState) {
		...
		var velocity = gestureState.vx * this._progressFactor;
		...
		Animated.sequence([
			...
			Animated.timing(this.openProgress, { toValue, bounciness:0, duration: 100}), 
